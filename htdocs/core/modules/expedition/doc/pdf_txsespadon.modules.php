...
/**
 *	\file       htdocs/core/modules/expedition/doc/pdf_txsespadon.modules.php
 *	\ingroup    expedition
 *	\brief      Class file allowing TxsEspadon shipping template generation
 */
...
/**
 *	Class to build sending documents with model TxsEspadon
 */
class pdf_txsespadon extends ModelePdfExpedition
{
	/**
	...
	public function __construct(DoliDB $db)
	{
		...
		$this->db = $db;
		$this->name = "txsespadon";
		$this->description = 'TXS Extended PDF template';
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$ord_part_by_orderline = array();
		$part_product_cache = array();
		...
		if ($conf->expedition->dir_output) {
			...
			if (file_exists($dir)) {
				...
				if (!empty($notetoshow) || $tracking_number_combined !== '' || $has_ship_billing_pdf) {
					...
					if ($tracking_number_combined !== '') {
						...
						foreach ($tracking_numbers as $tracking_number_item) {
							$object->getUrlTrackingStatus($tracking_number_item);
							$tracking_urls[] = $object->tracking_url;
						}
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if (empty($line->special_code) || $line->special_code != SUBTOTALS_SPECIAL_CODE) {
						$orderline_id = 0;
						if (!empty($line->origin_line_id)) {
							$orderline_id = (int) $line->origin_line_id;
						} elseif (!empty($line->fk_elementdet)) {
							$orderline_id = (int) $line->fk_elementdet;
						}

						if ($orderline_id > 0) {
							if (!array_key_exists($orderline_id, $ord_part_by_orderline)) {
								$sql = "SELECT ord_part FROM ".MAIN_DB_PREFIX."commandedet_extrafields";
								$sql .= " WHERE fk_object = ".(int) $orderline_id;
								$sql .= " LIMIT 1";
								$resql = $this->db->query($sql);
								$ord_part_by_orderline[$orderline_id] = 0;
								if ($resql && $this->db->num_rows($resql) > 0) {
									$obj = $this->db->fetch_object($resql);
									if (!empty($obj->ord_part)) {
										$ord_part_by_orderline[$orderline_id] = (int) $obj->ord_part;
									}
								}
								if ($resql) {
									$this->db->free($resql);
							...
							$ord_part_id = (int) $ord_part_by_orderline[$orderline_id];
							if ($ord_part_id > 0) {
								if (!isset($part_product_cache[$ord_part_id])) {
									require_once DOL_DOCUMENT_ROOT.'/product/class/product.class.php';
									$part_product = new Product($this->db);
									$part_product_cache[$ord_part_id] = null;
									if ($part_product->fetch($ord_part_id) > 0) {
										if (getDolGlobalInt('MAIN_MULTILANGS')) {
											$part_product->getMultiLangs();
										}
										$part_product_cache[$ord_part_id] = array(
											'ref' => $part_product->ref,
											'label' => $part_product->label,
											'description' => $part_product->description,
											'multilangs' => $part_product->multilangs ?? array(),
										);
									}
								}

								if (!empty($part_product_cache[$ord_part_id])) {
									...
									$line_backup = array(
										...
										'multilangs' => $line->multilangs ?? null,
									);

									$part_label = $part_product_cache[$ord_part_id]['label'];
									$part_desc = $part_product_cache[$ord_part_id]['description'];
									$part_text = $part_desc !== '' ? $part_desc : $part_label;

									$line->label = $part_text;
									$line->fk_product = $ord_part_id;
									$line->product_ref = $part_product_cache[$ord_part_id]['ref'];
									$line->product_label = $part_text;
									$line->desc = $part_text;
									$line->description = $part_text;
									$line->multilangs = $part_product_cache[$ord_part_id]['multilangs'];
								}
							}
						}
					}
					...
					if ($line_overridden) {
						$line->label = $line_backup['label'];
						$line->fk_product = $line_backup['fk_product'];
						$line->product_ref = $line_backup['product_ref'];
						$line->product_label = $line_backup['product_label'];
						$line->desc = $line_backup['desc'];
						$line->description = $line_backup['description'];
						if (array_key_exists('multilangs', $line_backup) && $line_backup['multilangs'] !== null) {
							$line->multilangs = $line_backup['multilangs'];
						} else {
							unset($line->multilangs);
						}
					}
				}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$title = $outputlangs->transnoentities("ShipmentSheet");
		...
		*/
		...
		if ($showaddress) {
			...
			// Use CUSTOMER contact on order for recipient (name, address, phone)
			$usecontact = false;
			$contact_name = '';
			$arrayidcontact = array();
			if (!empty($object->origin_object)) {
				$arrayidcontact = $object->origin_object->getIdContact('external', 'CUSTOMER');
			}
			...
			if ($usecontact && !empty($object->contact)) {
				$contact_name = $outputlangs->convToOutputCharset($object->contact->getFullName($outputlangs, 1));
				if ($object->contact->socid > 0 && $object->contact->socid != $object->thirdparty->id) {
					$object->contact->fetch_thirdparty();
					if (!empty($object->contact->thirdparty)) {
						$thirdparty = $object->contact->thirdparty;
					}
				}
			}

			if ($usecontact && !empty($contact_name)) {
				$carac_client_name = $contact_name;
			} elseif ($usecontact && ($object->contact->socid != $object->thirdparty->id && (!isset($conf->global->MAIN_USE_COMPANY_NAME_OF_CONTACT) || getDolGlobalString('MAIN_USE_COMPANY_NAME_OF_CONTACT')))) {
				$carac_client_name = pdfBuildThirdpartyName($object->contact, $outputlangs);
			} else {
				$carac_client_name = pdfBuildThirdpartyName($thirdparty, $outputlangs);
			}

			$carac_client = pdf_build_address($outputlangs, $this->emetteur, $thirdparty, (!empty($object->contact) ? $object->contact : null), ($usecontact ? 1 : 0), 'targetwithdetails', $object);
			if ($usecontact && !empty($contact_name) && strpos($carac_client, $contact_name."\n") === 0) {
				$carac_client = dol_substr($carac_client, dol_strlen($contact_name) + 1);
			}
