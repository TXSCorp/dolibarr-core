			...
			while ($i < $num) {
				...
				if (!$objp->fk_product > 0) {
					...
					if ($remaintodispatch || !getDolGlobalString('SUPPLIER_ORDER_DISABLE_STOCK_DISPATCH_WHEN_TOTAL_REACHED')) {
						...
						print '<input id="qty'.$suffix.'" name="qty'.$suffix.'" type="number" min="0" step="any" class="width50 right qtydispatchinput" value="'.(GETPOSTISSET('qty'.$suffix) ? GETPOSTINT('qty'.$suffix) : (!getDolGlobalString('SUPPLIER_ORDER_DISPATCH_FORCE_QTY_INPUT_TO_ZERO') ? $remaintodispatch : 0)).'">';
						// --- End Customization ---
